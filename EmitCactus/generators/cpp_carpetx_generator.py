    ...
    def generate_schedule_ccl(self) -> ScheduleRoot:
        ...
        for fn_name, fn in sorted(self.thorn_def.thorn_functions.items()):
            ...
            if len(sync_var_groups) > 0:
                schedule_blocks.append(ScheduleBlock(
                    group_or_function=GroupOrFunction.Function,
                    name=Identifier('StateSync'),
                    at_or_in=AtOrIn.In,
                    schedule_bin=Identifier(fn_group_name),
                    description=String(f'Empty function for explicitly SYNCing variables read Everywhere in function `{fn_name}`. Generated by EmitCactus.'),
                    lang=Language.C,
                    sync=[Identifier(g) for g in sync_var_groups],
            ...
            if fn.schedule_target in [ScheduleBin.Evolve, ScheduleBin.SpecialEvolve]:
                ...
                if len(sync_var_groups) > 0:
                    schedule_blocks.append(ScheduleBlock(
                        ...
                        at_or_in=AtOrIn.In,
                        schedule_bin=Identifier(fn_group_name),
                        description=String(f'Empty function for explicitly SYNCing variables read Everywhere in function `{fn_name}` in InitEvolve. Generated by EmitCactus.'),
                        lang=Language.C,
                        sync=[Identifier(g) for g in sync_var_groups],
                        before=[Identifier(fn_name)]
                    ))
            ...
            if fn.schedule_target is ScheduleBin.Analysis:
                ...
                if len(sync_var_groups) > 0:
                    schedule_blocks.append(ScheduleBlock(
                        ...
                        at_or_in=AtOrIn.In,
                        schedule_bin=Identifier(fn_group_name),
                        description=String(f'Empty function for explicitly SYNCing variables read Everywhere in function `{fn_name}` in InitAnalysis. Generated by EmitCactus.'),
                        lang=Language.C,
                        sync=[Identifier(g) for g in sync_var_groups],
                        before=[Identifier(fn_name)]
                    ))
        ...
        schedule_blocks.append(ScheduleBlock(
            ...
            at_or_in=post_post_init_at_in,
            schedule_bin=post_post_init_bin,
            description=String('Empty function to SYNC the state vector. Generated by EmitCactus.'),
            lang=Language.C,
