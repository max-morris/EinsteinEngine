...
CACTUS_DIR=$(realpath "$CACTUS_DIR")
...
SIM_CONFIG=${COTTONMOUTH_SIM_CONFIG:-sim}
if [ "$SIM_CONFIG" = "" ]
then
    echo "COTTONMOUTH_SIM_CONFIG must be non-empty" >&2
    exit 12
fi

./simfactory/bin/sim build "$SIM_CONFIG" -j"$BUILD_JOBS" --thornlist cottonmouth.th |& tee make.out
...
then
    ...
    for test_dir in "${bssnok_test_data[@]}"
    do
...
then
    ...
    for test_dir in "${z4c_test_dirs[@]}"
    do
        ln -s "$test_dir" "$TARGET_TEST_DIR_Z4c"
    done
fi

if [ -d "$HOME/simulations/$SIM_CONFIG-testsuite" ]
then
    rm -r "$HOME/simulations/$SIM_CONFIG-testsuite"
fi
